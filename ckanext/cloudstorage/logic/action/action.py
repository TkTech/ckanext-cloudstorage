...
PREFIX = config['ckanext.cloudstorage.prefix']
DOMAIN = config['ckanext.cloudstorage.domain']
SERVICE_ACCOUNT_KEYPATH = config['ckanext.cloudstorage.service_account_key_path']
...
def organization_create(next_auth, context, data_dict):
    ...
    role = "sysadmin"
    group_email = PREFIX + name + "@" + DOMAIN
...
def organization_delete(next_auth, context, data_dict):
    ...
    group_email = PREFIX + group_name + "@" + DOMAIN
